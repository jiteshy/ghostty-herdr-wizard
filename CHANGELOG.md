...
- New "Nerd Font + icons" choice in stage 2, saved in `~/.ghostty-herdr-wizard/choices.env` so later runs and `--only` reuse it. Icons (the default) keeps today's setup. Plain text skips the Nerd Font and makes every consumer glyph-free: herdr's sidebar uses `dots`, eza drops `--icons` (aliases and the `p` / prefix m preview), starship uses its `plain-text-symbols` preset, yazi empties its icon rules and powerline separators, LazyVim's mini.icons uses ASCII, lazygit turns off Nerd Font icons, the Claude Code status line drops its icons, and Ghostty keeps its built-in font. Re-run the wizard after changing it to rewrite those configs
